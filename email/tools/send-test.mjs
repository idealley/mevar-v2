...
*/
import { flag, requireApiKey, loadConfig, readIssue } from "./lib.mjs";

async function main() {
  const args = process.argv.slice(2);
  const { envKey, product } = await loadConfig();
  const apiKey = requireApiKey(envKey);

  const to = flag(args, "--to");
  if (!to) throw new Error("Provide --to <address> (your own inbox).");
  ...
  if (text) text = text.split("{{{RESEND_UNSUBSCRIBE_URL}}}").join(placeholder);
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html,
      ...(text ? { text } : {}),
      ...(product.replyTo ? { reply_to: product.replyTo } : {}),
    }),
  });
