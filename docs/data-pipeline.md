...
| `65c-strip-branham-furniture.mjs` | English | takes the booklets' page headers ("18 THE SPOKEN WORD", "AN EXODUS 19") out of the text, where the PDF confirms each one |
...
`manifests/branham-restore-unaligned.json`. 65c runs after it on the same PDFs, before
